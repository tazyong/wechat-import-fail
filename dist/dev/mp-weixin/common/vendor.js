"use strict";
var require$$0 = require("side-channel");
...
const toDisplayString = (val) => {
  return isString(val) ? val : val == null ? "" : isArray$3(val) || isObject(val) && (val.toString === objectToString || !isFunction(val.toString)) ? JSON.stringify(val, replacer, 2) : String(val);
};
...
function removeInterceptorHook(interceptors2, interceptor) {
  ...
  Object.keys(interceptor).forEach((name) => {
    ...
    if (isArray$3(hooks) && isFunction(hook)) {
      remove(hooks, hook);
    }
  });
}
...
function normalizePropsOptions(comp, appContext, asMixin = false) {
  ...
  if (isArray$3(raw)) {
    ...
    for (const key in raw) {
      ...
      if (validatePropName(normalizedKey)) {
        const opt = raw[key];
        const prop = normalized[normalizedKey] = isArray$3(opt) || isFunction(opt) ? { type: opt } : opt;
        if (prop) {
...
var getSideChannel = require$$0__default["default"];
var utils$1 = utils$2;
...
var sentinel = {};
var stringify$1 = function stringify2(object, prefix, generateArrayPrefix, strictNullHandling, skipNulls, encoder, filter, sort, allowDots, serializeDate2, format, formatter, encodeValuesOnly, charset, sideChannel) {
  var obj = object;
